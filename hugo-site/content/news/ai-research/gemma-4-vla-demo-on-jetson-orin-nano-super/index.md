...
date: '2026-04-22T16:15:42.301363+00:00'
exported_at: '2026-04-22T16:15:44.502604+00:00'
...
structured_data:
  ...
  headline: Gemma 4 VLA Demo on Jetson Orin Nano Super
  keywords: []
  main_image: ''
  original_source: https://huggingface.co/blog/nvidia/gemma4
  publisher:
    logo: /favicon.ico
    name: GTCode
title: Gemma 4 VLA Demo on Jetson Orin Nano Super
updated_at: '2026-04-22T16:15:42.301363+00:00'
url_hash: f43124cab998d8b29e45c99b5efe8e3585daa93a
---
...
The full script for this tutorial lives on GitHub, in my
`Google_Gemma`
repo next to the Gemma 2 demos:

👉
**[github.com/asierarranz/Google\_Gemma](https://github.com/asierarranz/Google_Gemma)**
...
That single file (
`Gemma4_vla.py`
) is all you need. It pulls the STT/TTS models and voice assets from Hugging Face on first run.
...
and
`cmake`
are only needed if you go the native llama.cpp route (Option A in Step 4). The rest is for audio, webcam, and Python.
...
(native build) and
`Q4_K_S`
(Docker) run comfortably on the 8 GB board once you've done the cleanup above. But if you've got other stuff you can't kill and memory is still tight, you can drop one step down to a
`Q3`
quant, same model, a bit less smart, noticeably lighter. Just swap the filename in Step 4:

```
gemma-4-E2B-it-Q3_K_M.gguf   # instead of Q4_K_M
```

Honestly though, stick with
`Q4_K_M`
if you can. It's the sweet spot.
...
You need a running
`llama-server`
with Gemma 4 before launching the demo. We'll build
`llama.cpp`
natively on the Jetson, it gives the best performance and full control over the vision projector that the VLA demo needs.
...
The
`mmproj`
file is the vision projector. Without it Gemma can't see, so don't skip it.
...
tells
`llama-server`
to push all the model's layers onto the GPU (99 is just "as many as the model has"). If you ever run into memory issues, you can lower that number to offload fewer layers to GPU and the rest to CPU. For this setup though, all layers on GPU should work fine.
...
Look for your USB mic. In our case the C920 showed up as
`plughw:3,0`
.
...
This lists your PulseAudio sinks. Pick the one that matches your speaker, it'll be a long ugly name like
`alsa_output.usb-...`
. In my case it was
`alsa_output.usb-Generic_USB2.0_Device_20130100ph0-00.analog-stereo`
...
3. If the question needs vision, she calls
   `look_and_answer`
   , the script grabs a webcam frame and sends it back
...
flag on
`llama-server`
is what enables this, it activates Gemma's native tool-calling support.
...
, do the cleanup from Step 3 again. Close everything. This model fits on 8 GB, but you have to be tidy.
...
**First run is slow**
, normal. It's downloading models and generating voice prompts. Second run is fast.
...
If you don't care about the full VLA demo and just want to poke at Gemma 4 on your Jetson without building anything, there's a ready-to-go Docker image from Jetson AI Lab with
`llama.cpp`
pre-compiled for Orin:
...
One line, no compilation, and
`-hf`
pulls the GGUF from Hugging Face on first run. Hit
`http://localhost:8080`
with any OpenAI-compatible client and chat away.

Heads up: this Docker path is
**text-only**
. It doesn't load the vision projector, so it won't work with the VLA demo above. For the full webcam experience, stick with the native build in Step 4.
