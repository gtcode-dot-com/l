...
date: '2026-06-11T03:37:31.505732+00:00'
exported_at: '2026-06-11T03:37:34.294607+00:00'
feed: https://huggingface.co/blog/feed.xml
source_url: https://huggingface.co/blog/CohereLabs/introducing-north-mini-code
structured_data:
  about: []
  author: ''
  description: A Blog post by Cohere Labs on Hugging Face
  headline: 'Introducing North Mini Code: Cohere’s First Model For Developers'
  keywords: []
  main_image: ''
  original_source: https://huggingface.co/blog/CohereLabs/introducing-north-mini-code
  publisher:
    logo: /favicon.ico
    name: GTCode
title: 'Introducing North Mini Code: Cohere’s First Model For Developers'
updated_at: '2026-06-11T03:37:31.505732+00:00'
url_hash: 3b631680ec0a9301bad9396886deef58e476f189
---
...
[![image1-benchmark-results](https://cdn-uploads.huggingface.co/production/uploads/6361a793c12a09b8a3184bff/f8NXK5yKtc6xE-hJ4XWbd.png)](https://cdn-uploads.huggingface.co/production/uploads/6361a793c12a09b8a3184bff/f8NXK5yKtc6xE-hJ4XWbd.png)
...
[See here for the details of our benchmarking methodology.](#benchmarking-methodology)*
...
[1](#fn1)
It ranks among the strongest open-source coding models in its size class.
...
[![image2](https://cdn-uploads.huggingface.co/production/uploads/6361a793c12a09b8a3184bff/g-SYXPG1oIxHEwnItd3he.png)](https://cdn-uploads.huggingface.co/production/uploads/6361a793c12a09b8a3184bff/g-SYXPG1oIxHEwnItd3he.png)

***Figure 2:**
North Mini Code is a Mixture-of-Experts Transformer decoder with interleaved sliding-window self-attention and full self-attention.*

North Mini Code is a decoder-only Transformer-based sparse Mixture-of-Experts model. It uses our efficient attention implementation, interleaved between sliding-window attention with RoPE and global attention with no positional embeddings, in a 3:1 ratio [
[1](#ref1)
]. The feed-forward block is an MoE block with 128 experts, of which 8 are activated per token. Each expert block is an FFN block with SwiGLU activation. The router applies a sigmoid activation function to the logits before the top-k selection. We also use a single dense layer before the sparse layers.

## Post-Training for Coding Excellence

[![image3](https://cdn-uploads.huggingface.co/production/uploads/6361a793c12a09b8a3184bff/8DQUkAkjo7Afat2Z4L7ue.png)](https://cdn-uploads.huggingface.co/production/uploads/6361a793c12a09b8a3184bff/8DQUkAkjo7Afat2Z4L7ue.png)

***Figure 3:**
The post-training pipeline is made up of two phases of supervised fine-tuning (SFT) and a phase of agentic reinforcement learning with verifiable rewards (RLVR) targeting software engineering and terminal tasks.*
...
We used 64K and 128K context lengths for the first and second stages of SFT, respectively. This “long-to-longer” cascade approach (similar to [
[5](#ref5)
,
[6](#ref6)
...
*shorter*
final trajectories during evaluation than training on a truncated distribution up to 64K only.

Instead of optimising North Mini Code towards quantitative metrics during SFT, we adopted an approach strictly using SFT as
*priming for RLVR.*
...
Harness robustness improves model usability in realistic software development settings, where agents encounter diverse and unpredictable tooling environments. These environments differ not just in prompting but in fundamental tool-use modality, For instance, SWE-Agent [
[8](#ref8)
...
and
`task`
etc) returning structured JSON responses.

[![image4](https://cdn-uploads.huggingface.co/production/uploads/6361a793c12a09b8a3184bff/xPc4PSWREdLtTS62tfl8L.png)](https://cdn-uploads.huggingface.co/production/uploads/6361a793c12a09b8a3184bff/xPc4PSWREdLtTS62tfl8L.png)

***Figure 4:**
To power a variety of agentic coding harnesses, North Mini Code is exposed to a variety of coding harnesses during the second SFT stage.*
...
*continuously*
. Policy weights are exported into vLLM every few learner steps (K=4), so the sampler is at most slightly off-policy at any moment. The residual mismatch is then corrected at the loss level.

To unblock the learner process from waiting on the longest rollouts while simultaneously avoiding a misbalance of data distribution across tasks, we used a
*windowed*
First-in-First-Out (FIFO) queue (trainer↔sampler) [
[11](#ref11)
]: a small fraction at the head of the queue is consumed in completion order to drain stragglers, with the rest staying in input order. Empirically, this recovers most of the throughput of a completion-order scheme without measurably hindering training stability.

We train using CISPO [
[12](#ref12)
...
[13](#ref13)
] with stronger regularization. We aggregate the loss at the token level rather than the prompt level, so the gradient signal scales with trajectory length and long agentic traces (where most of the credit-assignment signal lives) are not down-weighted relative to short ones.
...
For Terminal-based tasks, we configure the agent with a simple ReAct harness employing a single terminal-use tool based on Harbor's Tmux session implementation [
[14](#ref14)
...
[![image5](https://cdn-uploads.huggingface.co/production/uploads/6361a793c12a09b8a3184bff/Oe79J_Vn3Lbi10oKlHiQi.png)](https://cdn-uploads.huggingface.co/production/uploads/6361a793c12a09b8a3184bff/Oe79J_Vn3Lbi10oKlHiQi.png)

***Figure 5:**
The multi-environment RL training run improves model performance on benchmarks like SWE-Bench Verified and Terminal-Bench v2. Learning curves are displayed on the left across the RLVR training process.*

**Higher performance and robustness with online RL –**
...
* **Code Explanation:**
  Models are asked to explain particular technical aspects of a given code repository within a README file, or directly to the user.
* **Code Editing:**
  Models are tasked to implement a feature based on an existing code base.
* **Data Visualization:**
  Given data samples, models are tasked to create certain visualizations with a particular framework; no additional code is given.
* **Implementation from Scratch:**
  Given only design specifications and the packages to use, models are tasked to create a project from scratch, focused primarily on front-end design.

Evaluators are provided with rubric-based scoring questions to help them assess individual response criteria and rate individual attempts first, before giving a final preference rating between the two model trajectories.
[2](#fn2)
We share evaluation results of North Mini Code, comparing the SFT checkpoint with the final model release checkpoint.

[![image6](https://cdn-uploads.huggingface.co/production/uploads/6361a793c12a09b8a3184bff/-85CjexGqOFX5bahIdqxx.png)](https://cdn-uploads.huggingface.co/production/uploads/6361a793c12a09b8a3184bff/-85CjexGqOFX5bahIdqxx.png)

***Figure 6:**
Pairwise preference results for human evaluation comparing the final North Mini Code checkpoint after RLVR against the SFT-only checkpoint across 85 samples.*
...
North Mini Code models are available in OpenCode, Cohere API, and in HuggingFace with BF16 and FP8 (quantized) weights:
[bf16](https://huggingface.co/CohereLabs/North-Mini-Code-1.0)
,
[fp8](https://huggingface.co/CohereLabs/North-Mini-Code-1.0-fp8)
...
[16](#ref16)
], which requires strong algorithmic reasoning capabilities for coding performance outside of tool use. We run each benchmark with 3 different seeds and report the average benchmark performance, using temperature=1.0 and top\_p=0.95.
...
[17](#ref17)
]. For benchmark results that any public report is missing, denoted by (\*) in Figure 1, we run them internally using the recommended model configuration.
...
[7]
[Terminal-Bench: A Benchmark for AI Agents in Terminal Environments](https://github.com/laude-institute/terminal-bench)
...
[14]
[Harbor: A Framework for Evaluating and Optimizing Agents and Models in Container Environments](https://github.com/laude-institute/harbor)
...
1.
[AAII Coding Index](https://artificialanalysis.ai/models/capabilities/coding)
includes Terminal Bench Hard as an agentic coding task and SciCode as code generation benchmark for scientific problems.
[↩](#fnref1)

2. Both individual ratings and preferences are assessed on a five-point Likert scale.
[↩](#fnref2)