...
date: '2026-06-10T03:42:45.293484+00:00'
exported_at: '2026-06-10T03:42:46.630325+00:00'
...
structured_data:
  ...
  keywords: []
  main_image: ''
  original_source: https://huggingface.co/blog/adding-mcp-tools-to-reachy-mini
  publisher:
    logo: /favicon.ico
    name: GTCode
title: Adding MCP Tools to Reachy Mini
updated_at: '2026-06-10T03:42:45.293484+00:00'
...
A tool in the code isn't usable until it's enabled in
**a profile**
...
You can also write your own tool: add a Python file to the profile (or
`external_tools/`
), give it a name and description, and list that name in
`tools.txt`
.

Today there are built-in tools and custom local tools, and
`tools.txt`
decides which are active. This works well for the robot's body and keeps the trusted core small.

## The limits of local tools

The constraint here is that every tool has to be local Python. For
`move_head`
or
`play_emotion`
...
* you can still use custom one-off tools from
  `external_tools/`
...
Now the robot can search the web and check the weather in the same conversation, which is exactly what the
`canary_web_search_weather`
profile below does.
...
`add`
validates the Space on the Hub, probes the MCP endpoint, discovers its tools, and by default appends the tool IDs to the active profile's
`tools.txt`
...
&gt; `tools.txt`
&gt; is the gatekeeper: a remote tool is only active if its ID appears in the profile's
&gt; `tools.txt`
...
There is also a duplicate safety check at registry level:
`Tool.name`
...
What we're most curious about now is what people build. If you publish a tool Space, tag it
`reachy-mini-tool`
and
`mcp`
...
[Fabien Danieau](https://huggingface.co/FabienDanieau)
...
[Remi Fabre](https://huggingface.co/RemiFabre)
and the Pollen Robotics team for the ideas and feedback that shaped the remote tools workflow.*