...
date: '2026-05-06T20:15:36.965326+00:00'
exported_at: '2026-05-06T20:15:39.252417+00:00'
...
structured_data:
  ...
  keywords: []
  main_image: ''
  original_source: https://huggingface.co/blog/ServiceNow-AI/correctness-before-corrections
  publisher:
    logo: /favicon.ico
    name: GTCode
title: 'vLLM V0 to V1: Correctness Before Corrections in RL'
updated_at: '2026-05-06T20:15:36.965326+00:00'
url_hash: 08c10eedccbd3d4b0c4e2841d0e40f6a6ba7099f
---

# vLLM V0 to V1: Correctness Before Corrections in RL

[PipelineRL](/blog/ServiceNow-AI/github.com/ServiceNow/PipelineRL/)

uses vLLM as the inference engine for rollout generation. The
...
Figure 1. Trainer-side metrics for the vLLM V0 reference (blue), the initial vLLM V1 attempt (red), and the final vLLM V1 run after our fixes (green), including the fp32
`lm_head`
. The final V1 run returns close to the V0 trajectory across clip rate, KL, entropy, and reward.
...
1. **Semantic mismatch**
   : the backend returns logprobs with different meaning
   relative to what the trainer expects.
2. **Inference-path mismatch**
   : the backend uses different runtime defaults for
   caching, scheduling, or request handling, so the same prompts follow a
   different execution path.
3. **Objective mismatch**
   : the RL objective needs correction for the amount of
...
The policy-ratio plot shows this directly. Once
`processed_logprobs`
is on for
V1, the mean policy ratio stays centered extremely close to
`1.0`
across all
...
* prefix caching, left unset in the early run so the vLLM
  ...
  default
  applied
* async scheduling, left unset in the early run so the vLLM
  `0.18.1`
  default
  applied
* an ad-hoc
  `disable-cascade-attn`
  override that was set through launch-time
...
* `mode="keep"`
  matches the old inflight update model more closely than
  `wait`
  or
  `abort`
* `clear_cache=False`
  matches the V0 wrapper behavior, which left cached state
  intact on update
...
## The Remaining Gap: fp32 lm\_head
...
online RL. The
[ScaleRL paper](https://arxiv.org/abs/2510.13786)
later includes fp32
...
Figure 6. Reward for the vLLM V0 reference (blue), the initial vLLM V1 attempt (red), and the final vLLM V1 run with the fp32
`lm_head`
path (green). With the fp32 head included, the final V1 run tracks the V0 reference.
...
* **`processed_logprobs`
  alone**
  : fixed the semantic logprob bug; the training
  mismatch remained.
* **Batch invariance**
  : the mismatch remained in a separate test, with higher
  lag, higher clip rate, and NCCL complications.
* **Treating the first V1 run as a fair baseline**
  : the first V1 run had
