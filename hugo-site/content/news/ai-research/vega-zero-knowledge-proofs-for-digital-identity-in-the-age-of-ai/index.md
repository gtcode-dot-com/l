...
date: '2026-05-23T03:04:49.910561+00:00'
exported_at: '2026-05-23T03:04:53.461890+00:00'
...
structured_data:
  about: []
  author: ''
  description: Vega turns a full credential into a single proof, sharing only what
    is needed and nothing more, with performance that works in real apps.
  headline: 'Vega: Zero-knowledge proofs for digital identity in the age of AI'
  inLanguage: en
  keywords: []
  main_image: ''
  original_source: https://www.microsoft.com/en-us/research/blog/vega-zero-knowledge-proofs-for-digital-identity-in-the-age-of-ai
  publisher:
    logo: /favicon.ico
    name: GTCode
title: 'Vega: Zero-knowledge proofs for digital identity in the age of AI'
updated_at: '2026-05-23T03:04:49.910561+00:00'
url_hash: 5a6499e4ffd74c17b86b951cc50ea2e9c0c3e166
---

![Three white line-style icons centered on a textured blue-to-green gradient background: a shield, a checkmark inside a circle, and an ID card with a user silhouette—representing security, verification, and digital identity.](https://www.microsoft.com/en-us/research/wp-content/uploads/2026/05/VEGA-BlogHeroFeature-1400x788-1-1024x576.jpg)
...
We have spent several years working toward a practical solution. Privacy-preserving identity has been a throughout, and Vega’s proof system draws on several building blocks from that line of work:

* **[Spartan
  (opens in new tab)](https://eprint.iacr.org/2019/550)**
  showed how to efficiently prove R1CS, a standard way to express statements for a general-purpose proof system, with succinct proofs and without a trusted setup.
* [**Nova**

  (opens in new tab)](https://eprint.iacr.org/2021/370)
  introduced folding schemes, which let a prover compress many instances of a computation into one.
* **[HyperNova
  (opens in new tab)](https://eprint.iacr.org/2023/573)**
  showed that Nova’s folding also provides a key building block for zero-knowledge: folding a real instance with a random instance hides the underlying secret data, a technique dubbed “NovaBlindFold.”
* **[NeutronNova
  (opens in new tab)](https://eprint.iacr.org/2024/1605)**
...
Vega takes advantage of this structure by by splitting the prover’s secret data into three parts. The
*shared*
data (SHA-256 tables) and the
*precommitted*
part, such as the issuer signature and field locationsm are computed and committed once when the credential is first loaded. The
*online*
part, such as the device signature and  today’s date, is committed fresh each time. Before each proof, the precomputed commitments are refreshed with new randomness, which is cheaper than recomputing them and ensures that two proofs about the same credential cannot be linked.
...
Vega is implemented in Rust and will be open sourced soon. The proof system powering Vega is already available as the open-source
[spartan2
(opens in new tab)](https://github.com/microsoft/spartan2)
project on GitHub. The
[paper,](https://www.microsoft.com/en-us/research/publication/vega-low-latency-zero-knowledge-proofs-over-existing-credentials/)
joint work with Darya Kaviani, will be presented at the upcoming IEEE Symposium on Security and Privacy in San Francisco.
