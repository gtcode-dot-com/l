# OAHU UNDERGROUND — NETWORK MAP: MASTER NODE LIST (v4)
...
First appears: Issue 1
Key edges: BF-0 (son-in-law); BF-1 (brother-in-law); JD-2 (spouse); BF-2 (disclosed financial holdings >$2M in Luke entities); JD-3 (financial disclosures filed with); OV-1 (served — title/date TBD; exact role to be confirmed against Commission records and published in Evidence Locker upon verification); NP-3, NP-8 (board roles); JD-5 (Dec 2, 2022 hearing — Issue 4)
...
| ▸ (arrow, distinct weight/color) | Complaint / filing | JD-5 ▸ OV-1 (complaint filed) |
...
**Note on arrow types:** Employment arrows (──▶) and complaint arrows (▸) both use directional indicators but must be visually distinguishable in the printed map. Convention: employment arrows use the same line weight as governance edges; complaint arrows use a lighter weight or distinct color (e.g., red or grey). The graphic designer should ensure these cannot be confused at a glance.
...
| JD-1 | JD-3 | filing | disclosure filer | annual | Ethics Commission filing |
...
| JD-1 | JD-3 | filing | disclosure filer | "filer" | Ethics Commission filing |
...
- Central spine: the family edges (BF-0 ═══ BF-1, BF-0 ═══ JD-2, BF-1 ═══ JD-2, JD-1 ═══ JD-2) plus the financial edge (JD-1 ┄┄ BF-2)
